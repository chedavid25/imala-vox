...
export function CanalBadge({ canal, className, showIcon = true, showText = true }: CanalBadgeProps) {
  ...
  // Modo icono circular (showText=false): fondo de marca + icono blanco
  if (!showText) {
    return (
      <span
        className={cn(
          "inline-flex items-center justify-center rounded-full p-[3px]",
          canal === 'instagram'
            ? ""   // gradient vía style
            : c.bg,
          className
